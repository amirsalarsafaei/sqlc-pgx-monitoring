// Code generated by mockery; DO NOT EDIT.
// github.com/vektra/mockery
// template: testify

package metric

import (
	"context"

	mock "github.com/stretchr/testify/mock"
	"go.opentelemetry.io/otel/metric"

	_metricotelembedded "go.opentelemetry.io/otel/metric/embedded"
	_traceotelembedded "go.opentelemetry.io/otel/trace/embedded"
)

// NewMockInt64Counter creates a new instance of MockInt64Counter. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockInt64Counter(t interface {
	mock.TestingT
	Cleanup(func())
}) *MockInt64Counter {
	mock := &MockInt64Counter{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}

// MockInt64Counter is an autogenerated mock type for the Int64Counter type
type MockInt64Counter struct {
	mock.Mock
	_metricotelembedded.Meter
	_metricotelembedded.MeterProvider
	_traceotelembedded.Span
	_traceotelembedded.Tracer
	_traceotelembedded.TracerProvider
	_metricotelembedded.Float64Histogram
}

type MockInt64Counter_Expecter struct {
	mock *mock.Mock
}

func (_m *MockInt64Counter) EXPECT() *MockInt64Counter_Expecter {
	return &MockInt64Counter_Expecter{mock: &_m.Mock}
}

// Add provides a mock function for the type MockInt64Counter
func (_mock *MockInt64Counter) Add(ctx context.Context, incr int64, options ...metric.AddOption) {
	// metric.AddOption
	_va := make([]interface{}, len(options))
	for _i := range options {
		_va[_i] = options[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, ctx, incr)
	_ca = append(_ca, _va...)
	_mock.Called(_ca...)
	return
}

// MockInt64Counter_Add_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Add'
type MockInt64Counter_Add_Call struct {
	*mock.Call
}

// Add is a helper method to define mock.On call
//   - ctx context.Context
//   - incr int64
//   - options ...metric.AddOption
func (_e *MockInt64Counter_Expecter) Add(ctx interface{}, incr interface{}, options ...interface{}) *MockInt64Counter_Add_Call {
	return &MockInt64Counter_Add_Call{Call: _e.mock.On("Add",
		append([]interface{}{ctx, incr}, options...)...)}
}

func (_c *MockInt64Counter_Add_Call) Run(run func(ctx context.Context, incr int64, options ...metric.AddOption)) *MockInt64Counter_Add_Call {
	_c.Call.Run(func(args mock.Arguments) {
		var arg0 context.Context
		if args[0] != nil {
			arg0 = args[0].(context.Context)
		}
		var arg1 int64
		if args[1] != nil {
			arg1 = args[1].(int64)
		}
		var arg2 []metric.AddOption
		variadicArgs := make([]metric.AddOption, len(args)-2)
		for i, a := range args[2:] {
			if a != nil {
				variadicArgs[i] = a.(metric.AddOption)
			}
		}
		arg2 = variadicArgs
		run(
			arg0,
			arg1,
			arg2...,
		)
	})
	return _c
}

func (_c *MockInt64Counter_Add_Call) Return() *MockInt64Counter_Add_Call {
	_c.Call.Return()
	return _c
}

func (_c *MockInt64Counter_Add_Call) RunAndReturn(run func(ctx context.Context, incr int64, options ...metric.AddOption)) *MockInt64Counter_Add_Call {
	_c.Run(run)
	return _c
}

// int64Counter provides a mock function for the type MockInt64Counter
func (_mock *MockInt64Counter) int64Counter() {
	_mock.Called()
	return
}

// MockInt64Counter_int64Counter_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'int64Counter'
type MockInt64Counter_int64Counter_Call struct {
	*mock.Call
}

// int64Counter is a helper method to define mock.On call
func (_e *MockInt64Counter_Expecter) int64Counter() *MockInt64Counter_int64Counter_Call {
	return &MockInt64Counter_int64Counter_Call{Call: _e.mock.On("int64Counter")}
}

func (_c *MockInt64Counter_int64Counter_Call) Run(run func()) *MockInt64Counter_int64Counter_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockInt64Counter_int64Counter_Call) Return() *MockInt64Counter_int64Counter_Call {
	_c.Call.Return()
	return _c
}

func (_c *MockInt64Counter_int64Counter_Call) RunAndReturn(run func()) *MockInt64Counter_int64Counter_Call {
	_c.Run(run)
	return _c
}
